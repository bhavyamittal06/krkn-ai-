...
class TestFitnessCalculation:
    ...
    def test_calculate_fitness_for_new_scenario(self, genetic_algorithm_with_mock_runner):
        ...
        genetic_algorithm_with_mock_runner.krkn_client.run = Mock(return_value=mock_result)
        
        with patch.object(genetic_algorithm_with_mock_runner, 'save_scenario_result'):
            with patch.object(genetic_algorithm_with_mock_runner.health_check_reporter, 'plot_report'):
                with patch.object(genetic_algorithm_with_mock_runner.health_check_reporter, 'write_fitness_result'):
                    result = genetic_algorithm_with_mock_runner.calculate_fitness(scenario, generation_id)
                    
                    assert result.fitness_result.fitness_score == 15.5
                    assert scenario in genetic_algorithm_with_mock_runner.seen_population
                    genetic_algorithm_with_mock_runner.krkn_client.run.assert_called_once_with(scenario, generation_id)

    def test_calculate_fitness_for_seen_scenario(self, genetic_algorithm_with_mock_runner):
        ...
        # New generation ID
        new_generation_id = 1
        result = genetic_algorithm_with_mock_runner.calculate_fitness(scenario, new_generation_id)
